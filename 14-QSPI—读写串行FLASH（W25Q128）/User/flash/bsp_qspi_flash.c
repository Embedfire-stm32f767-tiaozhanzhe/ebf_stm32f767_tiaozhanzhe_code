...
/**
  * @brief  设置QSPI储存器为4字节地址模式
  * @retval QSPI存储器状态
  */
static uint8_t BSP_QSPI_4BYTEADDR_MODE(void)
{
  QSPI_CommandTypeDef s_command;
	/* 初始化复位使能命令 */
	s_command.InstructionMode   = QSPI_INSTRUCTION_1_LINE;
	s_command.Instruction       = ENTER_4BYTE_ADDRESS_MODE_CMD;
...
{ 
	QSPI_CommandTypeDef s_command;
	uint8_t value = W25Q256EJ_FSR_QE;
  ...
  /* 设置QSPI存储器为4字节地址模式 */
  if(BSP_QSPI_4BYTEADDR_MODE() != QSPI_OK)
  {
    return QSPI_ERROR;
  }
	...
	/* 自动轮询模式等待存储器就绪 */  
	if (QSPI_AutoPollingMemReady(W25Q256EJ_SUBSECTOR_ERASE_MAX_TIME) != QSPI_OK)
	{
		return QSPI_ERROR;
	}
	return QSPI_OK;
...
uint8_t BSP_QSPI_Write(uint8_t* pData, uint32_t WriteAddr, uint32_t Size)
{
	...
	{
		current_addr += W25Q256EJ_PAGE_SIZE;
	}
	...
	do
	{
		...
		pData += current_size;
		current_size = ((current_addr + W25Q256EJ_PAGE_SIZE) > end_addr) ? (end_addr - current_addr) : W25Q256EJ_PAGE_SIZE;
...
uint8_t BSP_QSPI_Erase_Chip(void)
{
	...
	/* 配置自动轮询模式等待擦除结束 */  
	if (QSPI_AutoPollingMemReady(W25Q256EJ_BULK_ERASE_MAX_TIME) != QSPI_OK)
	{
		return QSPI_ERROR;
	}
	return QSPI_OK;
...
{
	/* 配置存储器配置结构 */
	pInfo->FlashSize          = W25Q256EJ_FLASH_SIZE;
	pInfo->EraseSectorSize    = W25Q256EJ_SUBSECTOR_SIZE;
	pInfo->EraseSectorsNumber = (W25Q256EJ_FLASH_SIZE/W25Q256EJ_SUBSECTOR_SIZE);
	pInfo->ProgPageSize       = W25Q256EJ_PAGE_SIZE;
	pInfo->ProgPagesNumber    = (W25Q256EJ_FLASH_SIZE/W25Q256EJ_PAGE_SIZE);
	return QSPI_OK;
}
...
static uint8_t QSPI_WriteEnable()
{
	...
	/* 配置自动轮询模式等待写启用 */  
	s_config.Match           = W25Q256EJ_FSR_WREN;
	s_config.Mask            = W25Q256EJ_FSR_WREN;
...
static uint8_t QSPI_AutoPollingMemReady(uint32_t Timeout)
{
	...
	s_config.Match           = 0x00;
	s_config.Mask            = W25Q256EJ_FSR_BUSY;
	s_config.MatchMode       = QSPI_MATCH_MODE_AND;
	s_config.StatusBytesSize = 1;
	s_config.Interval        = 0x10;
	s_config.AutomaticStop   = QSPI_AUTOMATIC_STOP_ENABLE;
