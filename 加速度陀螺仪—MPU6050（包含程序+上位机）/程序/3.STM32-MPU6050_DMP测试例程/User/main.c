			  ...
			  if (inv_get_sensor_type_euler(data, &accuracy,(inv_time_t*)&timestamp))
						{
							...
							Send_Data((int16_t *)&sensors.gyro.raw,(int16_t *)&sensors.accel.raw);
									...
									LCD_DisplayStringLine(5,(uint8_t* )cStr);			

									
									sprintf ( cStr, "Roll  :  %.4f  ", Roll );	//inv_get_sensor_type_euler读出的数据是Q16格式，所以左移16位.
									LCD_DisplayStringLine(6,(uint8_t* )cStr);	
  ...
  if (result) {
      MPL_LOGE("Could not initialize gyro.\n");
			LED_RED;
							...
							LCD_DisplayStringLine(2,(uint8_t* )"No MPU6050 detected! ");			//野火自带的16*24显示
							LCD_DisplayStringLine(3,(uint8_t* )"Please check the hardware connection! ");			//野火自带的16*24显示
				#endif
  }
	else
	{	
				LED_GREEN;
				#ifdef USE_LCD_DISPLAY
							LCD_DisplayStringLine(2,(uint8_t* )"MPU6050 detected! ");			
				#endif
	}
