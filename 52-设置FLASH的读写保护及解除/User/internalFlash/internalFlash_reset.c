...
void OptionByte_Info(void)
{
	...
	if((temp&OB_IWDG_STOP_ACTIVE) == OB_IWDG_STOP_ACTIVE)
		FLASH_DEBUG("默认值：停止模式时独立看门狗计数器保持计数");
	else
		FLASH_DEBUG("停止模式时独立看门狗计数器停止计数");
	if((temp&OB_IWDG_STDBY_ACTIVE) == OB_IWDG_STDBY_ACTIVE)
		FLASH_DEBUG("默认值：待机模式时独立看门狗计数器保持计数");
	else
		FLASH_DEBUG("待机模式时独立看门狗计数器停止计数");	
	if((temp&OB_DUAL_BOOT_DISABLE) == OB_DUAL_BOOT_DISABLE)
		FLASH_DEBUG("默认值：根据启动地址选项启动");
	else
		FLASH_DEBUG("如果引导地址处于flash状态，则始终从系统内存引导（Dual Bank启动模式），或RAM如果RAM中的引导地址选项");

	if((temp&OB_NDBANK_SINGLE_BANK) == OB_NDBANK_SINGLE_BANK)	
		FLASH_DEBUG("默认值：flash单块模式，256位宽");
	else
		FLASH_DEBUG("flash双块模式，128位宽");
